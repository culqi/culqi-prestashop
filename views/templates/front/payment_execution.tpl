...
{else} {if isset($payment_error)}
    ...
    <p style="margin-top:20px;">
        Usted ha elegido pagar con tarjeta de crédito o débito. Solo cuenta con 10 minutos para realizar el pago, de lo
        contrario este expirará.
    </p>
    <p style="margin-top:20px;">
    ...
    <p style="display:block; margin: 0 auto;text-align: center;" class="cart_navigation" id="cart_navigation">
        ...
        <a style="line-height: 30px;"
           ...
           class="culqi_btn_cancel button_large">{l s='Cancelar' mod='culqi'}</a>
    </p>
    ...
    <script>
        ...
        function culqi() {
            if (Culqi.token) {
                ...
                const installments = (Culqi.token.metadata.installments == undefined) ? 1 : Culqi.token.metadata.installments;
                $.ajax({
        ...
        function showResult(style, message) {
            ...
            $('#showresultcontent')
                .attr('class', '')
                .addClass(style).html(message);
        }

        function redirect() {
            location.href = fnReplace("{/literal}{$link->getModuleLink('culqi', 'postpayment', [], true)|escape:'htmlall':'UTF-8'}{literal}");
        }
        function redirectPending() {
            location.href = fnReplace("{/literal}{$link->getModuleLink('culqi', 'postpendingpayment', [], true)|escape:'htmlall':'UTF-8'}{literal}");
        }
